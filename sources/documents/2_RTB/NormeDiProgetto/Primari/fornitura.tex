...
\begin{itemize}
  ...
  \item Preventivo orario: per ciascuna coppia risora-ruolo, il responsabile deve inserire nella cella apposita le ore produttive previste;
  \item La tabelle "preventivo economico" si aggiorna dinamicamente.
\end{itemize}
...
\begin{itemize}
  \item Consuntivo orario: il team ha definito una formula dinamica che somma le ore produttive per la coppia risorsa-ruolo. In questo modo è possibile automatizzare il calcolo del consuntivo anche quando i membri del team assumono più ruoli;
  \item Ore rimanenti per la coppia risorsa-ruolo: viene calcolata la differenza tra le ore rimanenti al termine dello \glossario{sprint} precedente e le ore spese nello \glossario{sprint} attuale;
  ...
  \begin{itemize}
    ...
    \item Costo (in €);
    \item Delta costo consuntivo - preventivo: differenza tra il costo preventivato e quello effettivo.	
    \item Ore e budget spesi negli \glossario{sprint} precedenti;
    \item Ore e budget restanti.
  \end{itemize}
\end{itemize}
\par Il processo di esportazione di tabelle e grafici è il medesimo del preventivo. Tutte le tabelle e i grafici del consuntivo devono essere inseriti nel \PdP. Una volta completata la stesura del consuntivo nel \PdP, il responsabile deve aggiornare le variabili globali della dashboard:
\begin{itemize}
  \item Ultimo-Sprint: ID, dove ID è il numero dell'ultimo \glossario{sprint};
  \item Preventivo complessivo del progetto (da modificare qualora sia necessaria una ridistribuzione delle ore per ruolo):
  \begin{itemize}
    \item Ruolo;
    \item Ore per ruolo;
