...
\subsubsubsection{Struttura font-end}
Il front-end è contenuto nella cartella \texttt{frontend}; al suo interno contiene le seguenti cartelle principali:
\begin{itemize}
  \item \textbf{src/components}: componenti riutilizzabili;
  \item \textbf{src/components/layout}: componenti che definiscono la struttura generale dell'applicazione, come header, footer, sidebar;
  \item \textbf{src/views}: pagine principali;
  \item \textbf{src/services}: servizi per la gestione delle chiamate API;
  \item \textbf{src/assets}: risorse come font e css generale del layout;
  \item \textbf{src/locales}: traduzioni del contenuto dell'interfaccia in diverse lingue;
  \item \textbf{src/router}: definizione delle rotte dell'applicazione;
...
\subsubsection{Testing del codice}\label{testing}
