...
\begin{itemize}
  ...
  \item Il file \emph{.gitignore} evita il tracciamento di file ausiliari o artefatti di compilazione;
  \item La directory \emph{Logo} contiene le versioni ufficiali del logo del gruppo, in formato SVG e/o PNG;
  \item La directory \emph{sources} include il codice sorgente per la documentazione, separato in due directory: 
  \begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Rimozioni dei verbali esterni, poiché firmati dalla \glossario{Proponente}, dal build output;
    \item Pubblicazione dell'artefatto generato durante il processo di build;
    \item Clonazione del repository di destinazione (argo-swe.github.io) in una directory temporanea accessibile tramite token;
