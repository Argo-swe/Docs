...
\par A seguire le analisi a posteriori del primo \glossario{sprint}:
\begin{itemize}
...
\par Per quanto riguarda il ruolo di analista, le ore non impiegate saranno reinvestite nello \glossario{sprint} a ridosso della \glossario{RTB}, al fine di rivedere i \glossario{casi d'uso} e apportare dei miglioramenti formali al documento di \AdR. L'ora produttiva di progettista, invece, sarà assegnata nel quarto \glossario{sprint}, per supportare l'attività di definizione dell'interfaccia grafica. Durante la \glossario{retrospettiva} del primo periodo, il team ha evidenziato anche la necessità di velocizzare la fase di approvazione delle modifiche. Di conseguenza, si è deciso di aggiungere una risorsa al ruolo di verificatore nel prossimo \glossario{sprint}.

\vspace{0.5\baselineskip}
\par Relativamente al calendario di massima del progetto, il gruppo non ha ritenuto di dover rielaborare la data stimata per la \glossario{RTB}. Nei prossimi \glossario{sprint}, invece, verrà valutata la possibilità di designare alcune ore di progettista al ruolo di amministratore.

\paragraph*{Gestione dei rischi (\sezione{sec:analisi_rischi}):}
\par Durante il primo \glossario{sprint}, il team non ha rilevato l'affioramento di rischi inattesi rispetto a quanto analizzato in fase di pianificazione. Tuttavia, alcune contromisure non si sono rivelate sufficienti a mitigare i rischi individuati, perciò il gruppo ha deciso di aggiornare la sezione di analisi dei rischi. Inoltre, il gruppo ha deciso di rivedere la probabilità di occorrenza e l'impatto di ciascun rischio, per garantire una maggiore coerenza con la situazione attuale.
